...
public abstract class LoopManiaWorldLoader {
    ...
    public LoopManiaWorldLoader(String filename, int difficulty) throws FileNotFoundException {
    ...
    public LoopManiaWorld load() {
        ...
        addRareItems(world, rareItemsJSON);
        ...
        Goals goal = gp.createGoal(jsonGoalCondition);
        world.setGoal(goal);
        world.setDifficulty(worldDifficulty);
        world.initialiseShop(worldDifficulty);
        return world;
    }
    ...
    private void loadEntity(LoopManiaWorld world, JSONObject currentJson, List<Pair<Integer, Integer>> orderedPath) {
        ...
        switch (type) {
            case "hero_castle":
                ...
                Building building = new HeroCastleBuilding(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y));
                onLoad(building);
    ...
    private void addRareItems(LoopManiaWorld world, JSONArray rareItemsJSON) {
        List<String> rareItems = new ArrayList<>();

        for (int i = 0; i < rareItemsJSON.length(); i++) {
            rareItems.add(rareItemsJSON.getString(i));
        }
